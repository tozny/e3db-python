...
class Client:
    ...
    @classmethod
    def register(self, registration_token, client_name, public_key, private_key=None, backup=False, api_url=DEFAULT_API_URL, public_signing_key="", private_signing_key=""):
        """
        ...
        public_signing_key : str
            base64urlencoded public_signing_key object obtained from
            e3db.Client.generate_keypair(). Optional. 

        private_signing_key : str
            base64urlencoded private_signing_key object obtained from
            e3db.Client.generate_keypair(). Optional. 
        ...
        if backup:
            ...
            # Backwards compatibility: Backups for existing v1 clients
            # should not include signing keys? (Even if set to default values)
            if public_signing_key == "":
                config = Config(
